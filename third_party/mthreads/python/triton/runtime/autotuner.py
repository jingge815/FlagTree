...
class Config:
    ...
    def __init__(self, kwargs, num_warps=4, num_stages=2, num_ctas=1, num_buffers_warp_spec=0, num_consumer_groups=0,
        ...
        try:
            ...
            capability = torch_musa.get_device_capability(device)
            if capability[0] < 3 and self.num_warps > 8:
                import logging
                logging.warning(
                    f"QY1/QY2 has 128 threads per warp, one should limit number of warps not exceed 8, so decrease self.num_warps: {self.num_warps} to 8"
                )
                self.num_warps = 8
        except ImportError:
            pass
